#!/usr/bin/env python3
"""
strategies.pipeline_hourly.profile — pipeline prod horaire
=========================================================
...
Usage :
    ...
    # Tout activer
    python train.py pipeline --data data/BTCUSD_1h_features.csv --mode combined

    # Pipeline short seul
    python train.py pipeline --data data/BTCUSD_1h_features.csv --mode short

    # Désactiver les shorts même en mode combined
    python train.py pipeline --data data/... --mode combined --no-short

    # Seuils explicites
    python train.py pipeline --data data/... --mode combined \\
...
Sorties dans  runs/pipeline/<run_id>/ :
    manifest.json            contrat canonique des artefacts
    ...
    risk/config.json         paramètres RiskController retenus
...
from core.risk import build_legacy_risk_config
from core.settings import configure_project_imports, get_settings

SETTINGS = get_settings()
FUTUR = configure_project_imports()
PROFILE = get_training_profile("pipeline")
...
from core.features.engineering import compute_flow_features, compute_long_features, compute_short_features
from core.features.live import MACRO_BUNDLE_COLS, compute_live_features, compute_macro_features
from core.labels import compute_short_reversal_col, compute_regime_col
from core.backtest import ShortRobustnessReport, run_wf_backtest_short, should_deploy_short
...
def make_risk_config(cfg: PipelineConfig, side: str) -> RiskConfig:
    ...
    if side == "long":
        return build_legacy_risk_config(
...
def write_component_metadata(path: Path, payload: Dict) -> None:
    json_dump(path / "metadata.json", payload)
...
def _ensure_pipeline_macro_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Garantit la présence des macro features attendues par les modèles legacy.

    Certaines sources prod/research ne contiennent que l'OHLCV brut. Dans ce cas,
    on injecte des valeurs neutres explicites ou des proxies déterministes pour
    conserver un contrat de features stable sans bloquer l'entraînement.
    """
    df = df.copy()

    def _zeros(name: str) -> None:
        if name not in df.columns:
            df[name] = 0.0

    def _series(name: str, values) -> None:
        if name not in df.columns:
            df[name] = pd.Series(values, index=df.index, dtype=np.float32)

    taker_ratio = pd.to_numeric(df.get("taker_buy_ratio_base"), errors="coerce") if "taker_buy_ratio_base" in df.columns else None
    taker_delta = pd.to_numeric(df.get("delta_taker_pressure"), errors="coerce") if "delta_taker_pressure" in df.columns else None
    ...
    if taker_delta is not None:
        ...
        _series("taker_ls_imbalance", ((taker_ratio.fillna(0.5) - 0.5) * 2.0))
    else:
        _zeros("taker_ls_imbalance")

    if taker_ratio is not None:
        ratio_mean = taker_ratio.rolling(24, min_periods=6).mean()
        ratio_std = taker_ratio.rolling(24, min_periods=6).std().clip(lower=1e-9)
        _series("taker_ls_buySellRatio_z_24", ((taker_ratio - ratio_mean) / ratio_std).fillna(0.0))
    elif taker_delta is not None:
        delta_mean = taker_delta.rolling(24, min_periods=6).mean()
        delta_std = taker_delta.rolling(24, min_periods=6).std().clip(lower=1e-9)
        _series("taker_ls_buySellRatio_z_24", ((taker_delta - delta_mean) / delta_std).fillna(0.0))
    else:
        _zeros("taker_ls_buySellRatio_z_24")

    _series(
        "funding_x_global_ls",
        pd.to_numeric(df["funding_rate_z_24"], errors="coerce").fillna(0.0)
        * pd.to_numeric(df["global_ls_longShortRatio_z_24"], errors="coerce").fillna(0.0),
    )
    _series(
        "oi_x_fng",
        pd.to_numeric(df["oihist_sumOpenInterest_z_24"], errors="coerce").fillna(0.0)
        * pd.to_numeric(df["fear_greed_value_z_24"], errors="coerce").fillna(0.0),
    )
...
def _is_enriched_1m_frame(df: pd.DataFrame) -> bool:
    required = {
        "datetime",
        "open",
        "high",
    ...
    if not required.issubset(df.columns):
        return False
    # Le pipeline 1h canonique attend `Close` + `future_ret_h`.
    # Si ces colonnes sont absentes mais le CSV expose des colonnes 1m enrichies,
    # on considère qu'il s'agit d'une table 1m à rééchantillonner.
    return "Close" not in df.columns and "future_ret_h" not in df.columns
...
def load_csv(path_arg: str) -> pd.DataFrame:
    ...
    # ── Détection auto du format brut 1m Binance (CSV) ───────────────────────
        ...
        if is_raw_1m:
            ...
            if _is_enriched_1m_frame(df.reset_index()):
                print("   CSV enrichi 1m détecté — rééchantillonnage vers 1h…")
                raw_1m = df.copy()
                raw_cols = [
                ...
                df = _raw1m_df_to_1h(raw_1m[raw_cols])

                macro_present = [c for c in _ENRICHED_1M_RESAMPLE_COLS if c in raw_1m.columns]
                if macro_present:
                    macro_1h = raw_1m[macro_present].resample("1h").last().ffill().fillna(0.0)
                    df = df.join(macro_1h, how="left")
                    df[macro_present] = df[macro_present].ffill().fillna(0.0)
...
def chronological_split(df: pd.DataFrame, test_from_year: int = 2024
    ...
    if train_mask.sum() == 0 or val_mask.sum() == 0:
        ...
        raise RuntimeError(
            "Split chronologique impossible pour le profil pipeline.\n"
            f"Couverture détectée: {year_min} → {year_max}.\n"
            f"Le pipeline prod exige un train ≤{TRAIN_END_YEAR} et une validation ={VAL_YEAR}.\n"
            "Utilise une source avec historique pré-2024 "
            "(ex: data/bundle_btc/raw/base_ohlcv.parquet) "
            "ou reconstruis features_merged.csv avec plus d'historique."
        )
...
def train_filter_model(df: pd.DataFrame,
    ...
    metrics = {
        ...
        "threshold_long": round(thr_long, 3),
        "threshold_short": round(thr_short, 3),
        "features": list(SNAPSHOT_FEATURES),
        "n_features": len(SNAPSHOT_FEATURES),
    }

    out_dir.mkdir(parents=True, exist_ok=True)
    dump_pickle(out_dir / "filter_model.pkl", clf)
    dump_pickle(out_dir / "filter_scaler.pkl", scaler)
    dump_pickle(out_dir / "model.pkl", clf)
    dump_pickle(out_dir / "scaler.pkl", scaler)
    json_dump(out_dir / "metrics.json", metrics)
    write_component_metadata(out_dir, {
        **metrics,
        "component": "filter",
        "schema_version": 2,
...
def main():
    ...
    validate_pipeline_label_stats(label_stats)
    ...
    if cfg.enable_short:
        short_result = train_short_model(
            ...
            train_mask=train_mask,
            val_mask=val_mask,
            out_dir=out / "edge_short",
        )
        trained_short_result = short_result
    ...
    short_enabled_for_inference = trained_short_result is not None and short_result is not None

    if long_result:
        write_component_metadata(out / "edge_long", {
            "schema_version": 2,
            "component": "edge_long",
            "side": "long",
            "model_name": long_result["best_model_name"],
            "best_tabular": long_result["best_tabular"],
            "threshold": round(cfg.direction_threshold_long, 4),
            "recommended_threshold": round(cfg.direction_threshold_long, 4),
            "features": list(long_result["features"]),
            "n_features": len(long_result["features"]),
            "enabled_for_inference": True,
            "has_calibrator": False,
            "model_file": "model.pkl",
            "scaler_file": "scaler.pkl",
            "legacy_model_file": "best_model.pkl",
        })

    if trained_short_result:
        write_component_metadata(out / "edge_short", {
            "schema_version": 2,
            "component": "edge_short",
            "side": "short",
            "model_name": trained_short_result["best_model_name"],
            "best_tabular": trained_short_result["best_tabular"],
            "threshold": round(short_threshold_calibrated, 4),
            "recommended_threshold": round(short_threshold_calibrated, 4),
            "features": list(trained_short_result["features"]),
            "n_features": len(trained_short_result["features"]),
            "enabled_for_inference": short_enabled_for_inference,
            "disabled_reason": short_disabled_reason,
            "has_calibrator": short_calibrator is not None,
            "calibrator_file": "calibrator.pkl" if short_calibrator is not None else None,
            "model_file": "model.pkl",
            "scaler_file": "scaler.pkl",
            "legacy_model_file": "best_model.pkl",
        })
    ...
    summary = {
        ...
        "edge_short_metrics":   trained_short_result["all_metrics"] if trained_short_result else None,
        "short_calibration": {
            "threshold_calibrated": short_threshold_calibrated,
            "method": "platt",
        } if trained_short_result else None,
    ...
    risk_config_doc = {
        ...
        "timeframe": TIMEFRAME,
        "horizon_minutes": HORIZON_MINUTES,
        "enabled_for_inference": {
            "long": bool(long_result),
            "short": short_enabled_for_inference,
        },
        "long": asdict(make_risk_config(cfg, "long")) if cfg.enable_long else None,
        "short": asdict(make_risk_config(cfg, "short")) if trained_short_result else None,
    }
    json_dump(out / "risk" / "config.json", risk_config_doc)

    manifest = {
        "schema_version": CANONICAL_SCHEMA_VERSION,
        "layout": CANONICAL_PIPELINE_LAYOUT,
        "pipeline": PROFILE.module_name,
        "run_id": run_id,
        "created_at_utc": created_at,
        "mode": mode,
        ...
        "components": {
            ...
            "edge_long": {
                ...
                "scaler": "scaler.pkl",
                "metadata": "metadata.json",
                "enabled_for_inference": bool(long_result),
            } if long_result else None,
            "edge_short": {
                "dir": "edge_short",
                "model": "model.pkl",
                "scaler": "scaler.pkl",
                "metadata": "metadata.json",
                "calibrator": "calibrator.pkl" if short_calibrator is not None else None,
                "enabled_for_inference": short_enabled_for_inference,
                "disabled_reason": short_disabled_reason,
            } if trained_short_result else None,
            "regime": {
                ...
                "metadata": "metadata.json",
                "enabled_for_inference": True,
            } if bear_regime_result else None,
            "risk": {
                "dir": "risk",
                "config": "config.json",
            },
            "backtest_long": {
                ...
                "present": long_bt is not None,
            },
            "backtest_short": {
                "dir": "backtest_short",
                "summary": "summary.json",
                "present": short_bt is not None,
            },
            "backtest_combined": {
                "dir": "backtest_combined",
                "summary": "summary.json",
                "present": combined_bt is not None,
            },
        },
    }
