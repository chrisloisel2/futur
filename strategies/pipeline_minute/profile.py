#!/usr/bin/env python3
"""
strategies.pipeline_minute.profile — pipeline R&D minute
========================================================
...
-----
    ...
    python train.py 1m --data ... --mode long --horizon 30

    # Run complet avec backtest walk-forward
    python train.py 1m --data ... --mode combined --wf
"""
...
DEFAULT_1M_PARQUET = SETTINGS.paths.data_dir / "bundle_btc" / "raw" / "base_ohlcv.parquet"
DEFAULT_1M_CSV_DIR = (
    SETTINGS.paths.data_dir
    / "datasets"
    / "binance_vision_downloads"
    / "data"
    / "spot"
    / "monthly"
    / "klines"
...
def _resolve_input_path(path: Path) -> Path:
    ...
    resolved = SETTINGS.paths.resolve(path)
    if resolved.exists():
        return resolved

    suggestions = []
    for candidate in (DEFAULT_1M_PARQUET, DEFAULT_1M_CSV_DIR):
        if candidate.exists():
            suggestions.append(str(candidate))

    msg = [f"Chemin dataset introuvable: {path}"]
    if resolved != path:
        msg.append(f"Chemin résolu essayé: {resolved}")
    if suggestions:
        msg.append("Exemples disponibles:")
        msg.extend(f"  - {candidate}" for candidate in suggestions)
...
def load_raw_1m(path: Path) -> pd.DataFrame:
    ...
    """
    path = _resolve_input_path(Path(path))
...
def parse_args():
    ...
    ap.add_argument("--data", default=str(DEFAULT_1M_PARQUET),
                    help="Parquet 1m local (défaut) ou répertoire de CSV bruts Binance Vision 1m")
    ap.add_argument("--out", default=str(SETTINGS.paths.pipeline_1m_runs_dir),
                    help="Dossier de sortie racine (canonique research)")
