#!/usr/bin/env python3
"""
delete_day_sessions.py — Supprime TOUTES les sessions d'une journée donnée.

Actions :
  1. Requête BDD pour lister toutes les sessions à la date spécifiée
...
Usage :
    python3 delete_day_sessions.py 2026-06-04
    python3 delete_day_sessions.py 2026-06-04 --yes        # pas de confirmation interactive
    python3 delete_day_sessions.py 2026-06-04 --dry-run    # affiche sans supprimer
...
def fetch_day_sessions(conn, date: str) -> list[dict]:
    """Récupère toutes les sessions dont started_at correspond à la date."""
...
def delete_on_disk(folder: str, sessions_dir: str, dry_run: bool) -> tuple[bool, str]:
    ...
    if not path.exists():
        return True, "absent du disque (déjà supprimé ou jamais présent)"
    if dry_run:
...
def main():
    parser = argparse.ArgumentParser(description="Supprime TOUTES les sessions d'une date donnée")
    ...
    print(f"\n{'[DRY-RUN] ' if args.dry_run else ''}Recherche de toutes les sessions du {args.date}...")

    conn = _pg_connect()
    sessions = fetch_day_sessions(conn, args.date)

    if not sessions:
        print(f"Aucune session trouvée pour le {args.date}.")
        conn.close()
        return
    ...
    for s in sessions:
        g = s["quality_grade"] or "non noté"
        grade_counts[g] = grade_counts.get(g, 0) + 1

    print(f"\n{len(sessions)} session(s) trouvée(s) pour le {args.date} :\n")
    ...
    for s in sessions:
        ...
        grade_str = f"{s['quality_grade']} ({s['quality_score']})" if s['quality_grade'] else "non noté"
        print(f"  • {s['session_id']}")
        print(f"    dossier    : {folder} ({disk_str})")
        print(f"    grade      : {grade_str} | started_at : {s['started_at']}")
    ...
    if not args.yes:
        print(f"\n⚠️  ATTENTION : ceci supprimera TOUTES les {len(sessions)} sessions du {args.date},")
        print("   tous grades confondus (A, B, C, D, F, non notées).")
