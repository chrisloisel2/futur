...
Architecture --once :
  ...
  3. ProcessPoolExecutor → lecture JSON + calcul score en parallèle par chunks
...
def _scan_chunk(args: tuple) -> list:
    """
    Worker process : lit analysis.json + config.json pour un lot de dossiers,
    calcule le score, retourne les résultats.

    args: (sessions_dir, [folder_name, ...])
    Retourne: [(folder_name, score, grade, errors, warnings_count, config, error_msg), ...]
    """
    ...
    for folder_name in folder_names:
        ...
        try:
            ...
            if not analysis_path.exists():
                results.append((folder_name, None, None, None, 0, None, "no_analysis"))
                continue
            ...
            results.append((folder_name, score, grade, errors, len(warnings), config, None))

        except Exception as exc:
            results.append((folder_name, None, None, None, 0, None, str(exc)))
...
def run_once():
    ...
    # ── Étape 1 : liste FS + déjà traités ──────────────────────────────────────
    all_sessions = _list_sessions(SESSIONS_DIR)
    already_done = _load_already_scored()
    to_scan = [f for f in all_sessions if f not in already_done]

    logger.info("Total dossiers : %d | déjà traités : %d | à scanner : %d",
                len(all_sessions), len(already_done), len(to_scan))
    ...
    with ProcessPoolExecutor(max_workers=SCAN_WORKERS) as pool:
        for chunk_results in pool.map(_scan_chunk, chunks, chunksize=1):
            ...
            for folder_name, score, grade, errors, warnings_count, config, err in chunk_results:
    ...
    logger.info("=== Terminé en %.1fs total ===", time.monotonic() - t0)
