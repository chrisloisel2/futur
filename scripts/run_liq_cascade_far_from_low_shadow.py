#!/usr/bin/env python3
"""
scripts/run_liq_cascade_far_from_low_shadow.py
─────────────────────────────────────────────────────────────────────────────
LIQ_CASCADE_FAR_FROM_LOW_V1 — Mode A (SIGNAL SHADOW) runner.

Live Alpha Lab, deuxième alpha implémenté (voir configs/live_alpha_registry.yaml
alpha_id: LIQ_CASCADE_FAR_FROM_LOW_V1). Calcule le signal, écrit une Opportunity
par décision — n'envoie AUCUN ordre, ne simule même pas de fill (Mode A pur).

Ne touche JAMAIS reports/liq_cascade/shadow/ (le ledger LIQ_CASCADE existant)
ni reports/live_alpha_lab/LIQ_CASCADE_REPEAT_V1/ (le ledger du premier alpha
Live Alpha Lab) : écrit dans son propre dossier
reports/live_alpha_lab/LIQ_CASCADE_FAR_FROM_LOW_V1/. Ne modifie JAMAIS
...
from src.institutional.engines.liq_cascade.far_from_low_variant import select_tradeable
...
def main() -> int:
    ...
    print(f"[{ALPHA_ID}] {len(ev)} events LIQ_CASCADE bruts -> "
          f"{len(tradeable)} far-from-low LONG_CASCADE tradeable", flush=True)
    ...
    dec = tradeable[["event_time", "symbol", "kind", "dist_low_24h",
                     "low_bucket", "direction", "oi_drop_z", "px_ret_30m"]].copy()
    dec["engine"] = ALPHA_ID
    dec["horizon"] = HORIZON
