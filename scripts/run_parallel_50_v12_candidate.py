#!/usr/bin/env python3
"""
scripts/run_parallel_50_v12_candidate.py
─────────────────────────────────────────────────────────────────────────────
Config candidate V1.2 = RANKED7_EDGE + les deux leviers validés séparément :
  • long_fraction 0.0645 (×1.5 — au-delà, ROI baisse et DD>3%)
  • carry_fraction 0.75  (frontière linéaire mesurée, DD insensible)

On MESURE la combinaison (pas d'additivité supposée) + son stress coûts ×2.
Gates : ROI>0, maxDD ≤ 3 %, cost×2 positif.
Sortie : reports/parallel_50/parallel_50_v12_candidate.json
"""
...
def cfg(**over):
    d = dict(
        initial_capital=CAPITAL, carry_fraction=0.75, max_open_longs=7, long_fraction=0.0645,
...
def main():
    ...
    print(f"Univers tradable: {len(tradable)}/{len(U)} | candidate V1.2 (long×1.5 + carry75)", flush=True)

    results = {}
    results["V12_CANDIDATE"] = run("V12_CANDIDATE", tradable, cfg())
    results["V12_COSTX2"] = run("V12_COSTX2", tradable,
                                cfg(taker_fee_bps=10.0, slippage_bps=4.0, maker_fee_bps=2.0))

    (ROOT/"reports/parallel_50/parallel_50_v12_candidate.json").write_text(
