...
def stack_equity_daily() -> pd.Series:
    ...
    for _, r in allt.iterrows():
        ...
        open_pos[r["symbol"]] = t + pd.Timedelta(hours=HOLD_H[r["engine"]])
...
def main():
    v12 = v12_equity()
    stk = stack_equity_daily()
    legs = {"V1.2 (socle)": v12, "STACK 3 moteurs (2%/trade)": stk}
    ...
    L = [f"# Overlay mesuré — {len(norm)} jambes\n",
    ...
    (DIR / "V12_PLUS_STACK_OVERLAY.json").write_text(
        json.dumps(out, indent=2, default=str))
    (DIR / "V12_PLUS_STACK_OVERLAY.md").write_text("\n".join(L))
    print("\n".join(L))
