...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Parallel worker processes. 0=auto (min(groups, cpu_count-1, 8))",
    )
    parser.add_argument(
        "--write-threads", type=int, default=4,
        help="MongoDB write threads per worker process",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        # ── Row estimation for accurate progress bar ──────────────────────
        LOG.info("Estimating row counts (%d groups)…", len(specs))
        row_estimates = _estimate_group_rows(specs)
        total_rows = max(sum(row_estimates.values()), 1)
        LOG.info("Estimated total rows: %s", f"{total_rows:,}")

        # ── Worker count ─────────────────────────────────────────────────
        n_workers = int(args.workers)
        if n_workers <= 0:
            n_workers = max(1, min(len(specs), (os.cpu_count() or 2) - 1, 8))
        write_threads = max(1, int(args.write_threads))
        LOG.info("workers=%d write_threads=%d per_worker", n_workers, write_threads)

        def _make_payload(spec: GroupSpec, prog_q: Any = None) -> Dict[str, Any]:
            return {
                "kind": spec.kind, "symbol": spec.symbol, "interval": spec.interval,
                "count": spec.count, "source_origin": spec.source_origin,
                "paths": [str(p) for p in spec.paths], "raw_symbol": spec.raw_symbol,
                "mongo_uri": args.mongo_uri, "mongo_db": args.mongo_db,
                "source_collection": args.source_collection,
                "target_collection": args.target_collection,
                "collection_mode": args.collection_mode,
                "horizons": horizons, "label_horizons": label_horizons,
                "timeframes": timeframes if not args.no_mtf else [],
        ...
        totals: Dict[str, int] = {
            "groups": 0, "skipped": 0, "raw_rows": 0, "clean_rows": 0,
            "written_rows": 0, "collection_writes": 0, "upserted": 0, "modified": 0, "matched": 0,
        }
        ...
        try:
            ...
            _HAS_TQDM = not args.no_progress
        except ImportError:
            _HAS_TQDM = False

        pbar = (
            _tqdm(
                total=total_rows, unit="rows", dynamic_ncols=True,
                desc="OHLCV build",
                bar_format=(
                    "{desc}: {percentage:3.1f}%|{bar:45}| "
                    "{n_fmt}/{total_fmt} rows "
        ...
        # ── Run: parallel (multi-group) or sequential (single group / debug) ──
        if n_workers > 1 and len(specs) > 1:
            # Each group → one worker process; chunk progress via Manager Queue
            mp_manager = multiprocessing.Manager()
            payloads = [_make_payload(s, mp_manager.Queue(maxsize=0)) for s in specs]

            # Background thread drains all worker queues → updates pbar
            _stop = threading.Event()

            def _drain() -> None:
                while not _stop.is_set() or any(not p["progress_q"].empty() for p in payloads):
                    for p in payloads:
                        try:
                            while True:
                                msg = p["progress_q"].get_nowait()
                                if msg[0] == "progress" and pbar:
                                    pbar.update(msg[1])
                                    _refresh_postfix()
                        except Exception:
                            pass
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as exe:
                futures = {exe.submit(_group_task, p): p["symbol"] + "/" + p["interval"] for p in payloads}
                for fut in as_completed(futures):
                    label = futures[fut]
                    try:
                        stats = fut.result()
                    except Exception as exc:
                        LOG.exception("worker FAILED %s: %s", label, exc)
                        stats = {"skipped": 1}
            ...
            # Sequential: all groups run in-process; progress updated per chunk
            local_q: queue.SimpleQueue = queue.SimpleQueue()

            def _local_drain() -> None:
                while not _stop_local.is_set() or not local_q.empty():
                    try:
            ...
            _stop_local = threading.Event()
            drain_local = threading.Thread(target=_local_drain, daemon=True)
            drain_local.start()

            for i, spec in enumerate(specs, 1):
                LOG.info("[%d/%d] %s %s", i, len(specs), spec.symbol, spec.interval)
                payload = _make_payload(spec, local_q)
                try:
                    stats = _group_task(payload)
                except Exception as exc:
                    LOG.exception("FAILED %s %s: %s", spec.symbol, spec.interval, exc)
                    stats = {"skipped": 1}
                accumulate(totals, stats)
                _refresh_postfix()
                LOG.info(
                    "cumulative groups=%d written=%d elapsed=%.1fs",
                    totals["groups"], totals["written_rows"], time.monotonic() - t0,
                )

            _stop_local.set()
            drain_local.join(timeout=2)
        ...
        metadata = {
            ...
            "include_labels": not args.no_labels, "include_mtf": not args.no_mtf,
            "fee_bps": args.fee_bps, "slippage_bps": args.slippage_bps,
            "triple_target_atr": args.triple_target_atr, "triple_stop_atr": args.triple_stop_atr,
            "start": start, "end": end,
            "dry_run": bool(args.dry_run), "audit_only": bool(args.audit_only),
            "workers": n_workers, "write_threads": write_threads,
            "totals": totals, "coverage": integrity,
        ...
        LOG.info("DONE totals=%s elapsed=%.1fs", json.dumps(totals, default=str), time.monotonic() - t0)
...
def _estimate_group_rows(specs: Sequence[GroupSpec]) -> Dict[Tuple[str, str], int]:
    """Fast row count via parquet footer metadata (no data loading)."""
    try:
        ...
        def _fast_count(path: Path) -> int:  # type: ignore[misc]
            return 0

    out: Dict[Tuple[str, str], int] = {}
    for spec in specs:
        key = (spec.symbol, spec.interval)
        if spec.kind == "local":
            out[key] = out.get(key, 0) + sum(_fast_count(p) for p in spec.paths)
        else:
            out[key] = out.get(key, 0) + int(spec.count)
    return out


def _group_task(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Top-level, picklable worker — one (symbol, interval) group per call.

    Creates its own MongoDB connection so it can safely run in a subprocess.
    Sends ("progress", n_rows) messages to payload["progress_q"] after each chunk.
    """
    import gc as _gc
    ...
    spec = GroupSpec(
        ...
        paths=tuple(Path(p) for p in payload.get("paths", [])),
        raw_symbol=payload.get("raw_symbol"),
    )
    prog_q = payload.get("progress_q")

    def _send(n: int) -> None:
        if prog_q is not None:
            try:
                prog_q.put_nowait(("progress", n))
            except Exception:
                pass

    client = MongoClient(payload["mongo_uri"], serverSelectionTimeoutMS=10_000, maxPoolSize=4)
    try:
        db = client[payload["mongo_db"]]
        source = db[payload["source_collection"]]

        start = payload.get("start")
        end = payload.get("end")

        raw = load_group_frame(source, spec, max_rows=payload.get("max_rows_per_group"), start=start, end=end)
        if raw is None or raw.empty:
            return {"skipped": 1, "groups": 0, "clean_rows": 0, "written_rows": 0}
        ...
        min_rows = int(payload.get("min_rows", 500))
        if len(clean) < min_rows:
            return {"skipped": 1, "groups": 0, "raw_rows": raw_rows, "clean_rows": len(clean), "written_rows": 0}

        dry_run = bool(payload.get("dry_run", False))
        collections_list = target_collections_for_symbol(
            db=db,
        ...
        label_horizons = payload["label_horizons"]
        include_labels = bool(payload.get("include_labels", True))
        max_feature_h = max(horizons or [1])
        max_label_h = max(label_horizons or [1]) if include_labels else 0
        ctx_override = payload.get("context_rows")
        context_before = int(ctx_override) if ctx_override else max(1000, max_feature_h * 5)
        context_after = max_label_h
        chunk_rows = int(payload.get("chunk_rows") or 25_000)
        batch_size = int(payload.get("batch_size", 2_000))
        write_threads = max(1, int(payload.get("write_threads", 4)))

        n = len(clean)
        totals: Dict[str, int] = {
            "groups": 1, "skipped": 0, "raw_rows": raw_rows, "clean_rows": n,
            "written_rows": 0, "collection_writes": 0, "upserted": 0, "modified": 0, "matched": 0,
        }

        for core_start in range(0, n, chunk_rows):
            ...
            chunk_size = core_end - core_start
            ...
            features = build_feature_matrix(
                ...
                include_mtf=bool(payload.get("include_mtf", False)),
                fee_bps=float(payload.get("fee_bps", 6.0)),
                slippage_bps=float(payload.get("slippage_bps", 2.0)),
                triple_target_atr=float(payload.get("triple_target_atr", 1.5)),
                triple_stop_atr=float(payload.get("triple_stop_atr", 1.0)),
            )
            features = features.loc[features.index.isin(write_index)].copy()

            if not features.empty:
                if dry_run:
                    totals["written_rows"] += len(features)
                else:
                    ws = _write_features_threaded(
                        features=features,
                        collections=collections_list,
                        batch_size=batch_size,
                        write_threads=write_threads,
                    )
                    for k in ("written_rows", "collection_writes", "upserted", "modified", "matched"):
                        totals[k] += ws.get(k, 0)
...
def _write_features_threaded(
    *,
    ...
    """Batch MongoDB upserts across multiple collections using a thread pool.

    One thread per (batch × collection) pair → saturates MongoDB write path
    ...
    # Pre-convert all batches to records once (avoid repeated serialisation)
    batches = [
        dataframe_to_mongo_records(features.iloc[i: i + batch_size])
        for i in range(0, len(features), batch_size)
    ]

    def _bulk(records: List[Dict[str, Any]], collection: Any) -> Tuple[int, int, int]:
        if not records:
            return 0, 0, 0
        ops = [
            ReplaceOne(
        ...
        try:
            ...
            LOG.error("BulkWriteError col=%s: %s", collection.name, str(exc.details)[:400])
            raise

    futures_n: Dict[Any, int] = {}
    with ThreadPoolExecutor(max_workers=write_threads) as executor:
        for records in batches:
            for col in collections:
                f = executor.submit(_bulk, records, col)
                futures_n[f] = len(records)
        for f in as_completed(futures_n):
            up, mod, matched = f.result()
            n = futures_n[f]
            stats["upserted"] += up
            stats["modified"] += mod
            stats["matched"] += matched
            stats["collection_writes"] += 1
            stats["written_rows"] += n

    return stats


# ── Legacy sequential path (kept for compatibility) ───────────────────────
...
def build_feature_matrix(
    ...
    *,
    ...
    for h in horizons:
        ...
        if h >= 2:
            # std/variance require ≥2 observations — always NaN for h=1, skip entirely
            out[f"realized_vol_{h}"] = log_ret.rolling(h, min_periods=minp).std()
        if h >= 4:
            # Directional vol: ~50% of bars per direction → need h≥4 for ~25% fill floor
            # h=2/3: P(all-same-direction) ≈ 25%/12.5% → unusable fill, skip
            dir_minp = min(max(2, h // 4), h)
            out[f"downside_vol_{h}"] = log_ret.where(log_ret < 0).rolling(h, min_periods=dir_minp).std()
            out[f"upside_vol_{h}"] = log_ret.where(log_ret > 0).rolling(h, min_periods=dir_minp).std()
        ...
        # rolling rank prevents look-ahead bias (global rank leaks future data)
        if h >= 2 and f"volume_zscore_{h}" in out:
            rank_window = max(h * 2, 200)
            out[f"absorption_proxy_{h}"] = out[f"volume_zscore_{h}"] - out[f"range_pct_{h}"].rolling(rank_window, min_periods=h).rank(pct=True)

        # pandas rolling skew requires ≥3 obs, kurt ≥4; skip h too small
        if h >= 3:
            out[f"return_skew_{h}"] = log_ret.rolling(h, min_periods=max(3, minp)).skew()
        if h >= 5:
            out[f"return_kurt_{h}"] = log_ret.rolling(h, min_periods=max(4, minp)).kurt()
        out[f"garman_klass_vol_{h}"] = garman_klass_vol(open_, high, low, close, h)
        out[f"yang_zhang_vol_{h}"] = yang_zhang_vol(open_, high, low, close, h)
...
def build_multi_timeframe_features(
    ...
    *,
    ...
    for tf in timeframes:
        ...
        # Minimum 10 resampled bars — allows 1d MTF from 1m chunks (~35 days)
        # with graceful NaN for large horizons (h=100 needs 60+ bars → NaN is fine)
        if len(resampled) < 10:
            continue
...
def build_light_indicator_set(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    n_bars = len(frame)
    # Require 6× headroom: ADX uses 3 successive EWMs, each needing ~2h bars to converge
    # e.g. 4h from 1m chunk (~213 bars): h=20 ✓ (120≤213), h=50 ✗ (300>213)
    # e.g. 1d from 1m chunk (~35 bars) : h=5 ✓ (30≤35), h=10 ✗ (60>35)
    safe_horizons = [h for h in (5, 10, 20, 50, 100) if h * 6 <= n_bars]
    if not safe_horizons:
        safe_horizons = [5] if n_bars >= 10 else []
    for h in safe_horizons:
        ...
        if h >= 2:
            out[f"volume_zscore_{h}"] = safe_div(
                volume - volume.rolling(h, min_periods=minp).mean(),
                volume.rolling(h, min_periods=minp).std(),
            )
        out[f"choppiness_{h}"] = choppiness_index(high, low, close, h)
        _, _, adx = adx_indicators(high, low, close, h)
        out[f"adx_{h}"] = adx

    # Composite scores use rolling(200) rank that requires ~50+ bars to converge —
    # skip in MTF light set to avoid low-fill features on small resampled chunks.
    # These composites are already present in the base feature matrix.
...
def triple_barrier(
    *,
    ...
    hi = high.to_numpy(dtype="float64")
    lo = low.to_numpy(dtype="float64")
    a = atr_pct.to_numpy(dtype="float64")
    n = len(c)

    valid = np.isfinite(c) & np.isfinite(a) & (a > 0)
    upper = np.where(valid, c * (1.0 + target_mult * a + cost_rate), np.nan)
    lower = np.where(valid, c * (1.0 - stop_mult * a - cost_rate), np.nan)

    label = np.full(n, np.nan, dtype="float64")
    tte = np.full(n, float(horizon), dtype="float64")
    # unlabeled = rows still waiting for a barrier hit
    unlabeled = valid.copy()
    ...
    for j in range(1, horizon + 1):
        ...
        hit_up = un & np.isfinite(hi_j) & (hi_j >= upper[:max_i])
        hit_dn = un & np.isfinite(lo_j) & (lo_j <= lower[:max_i])
        both = hit_up & hit_dn
        only_up = hit_up & ~hit_dn
        only_dn = hit_dn & ~hit_up
        any_hit = both | only_up | only_dn

        label[:max_i] = np.where(both, 0.0, np.where(only_up, 1.0, np.where(only_dn, -1.0, label[:max_i])))
        tte[:max_i] = np.where(any_hit, float(j), tte[:max_i])
        unlabeled[:max_i] &= ~any_hit

    # rows with no barrier hit: classify by horizon return
    max_i = n - horizon
    if max_i > 0:
        still = unlabeled[:max_i] & np.isfinite(c[horizon: horizon + max_i])
        r = np.where(still, (c[horizon: horizon + max_i] - c[:max_i]) / c[:max_i], np.nan)
        label[:max_i] = np.where(still & (r > cost_rate), 1.0,
                        np.where(still & (r < -cost_rate), -1.0,
                        np.where(still, 0.0, label[:max_i])))
...
def mfi(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series, n: int) -> pd.Series:
    n = max(1, int(n))
    # Clamp min_periods to window to avoid always-NaN for small h
    minp = min(min_periods(n), n)
    tp = (high + low + close) / 3.0
    money = tp * volume
    direction = tp.diff()
    pos = money.where(direction > 0, 0.0)
    neg = money.where(direction < 0, 0.0)
    pos_sum = pos.rolling(n, min_periods=minp).sum()
    neg_sum = neg.rolling(n, min_periods=minp).sum()
...
def garman_klass_vol(open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, n: int) -> pd.Series:
    """Garman-Klass OHLCV volatility estimator — ~8x more efficient than close-to-close."""
    n = max(2, int(n))
    log_hl = safe_log(safe_div(high, low))
    log_co = safe_log(safe_div(close, open_))
    gk = 0.5 * log_hl ** 2 - (2.0 * math.log(2) - 1.0) * log_co ** 2
    minp = min_periods(n)
    return gk.rolling(n, min_periods=minp).mean().pipe(lambda s: s.clip(lower=0)).apply(np.sqrt, raw=True)


def yang_zhang_vol(open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, n: int) -> pd.Series:
    """Yang-Zhang OHLCV volatility estimator — handles overnight gaps, most efficient estimator."""
    n = max(3, int(n))
    minp = min_periods(n)
    log_oc = safe_log(safe_div(close, open_))
    log_co_prev = safe_log(safe_div(open_, close.shift(1)))
    log_ho = safe_log(safe_div(high, open_))
    log_lo = safe_log(safe_div(low, open_))
    # Rogers-Satchell component (drift-independent)
    rs = (log_ho * (log_ho - log_oc) + log_lo * (log_lo - log_oc)).rolling(n, min_periods=minp).mean()
    # overnight variance
    oc_prev_mean = log_co_prev.rolling(n, min_periods=minp).mean()
    var_overnight = ((log_co_prev - oc_prev_mean) ** 2).rolling(n, min_periods=minp).mean()
    # open-to-close variance
    oc_mean = log_oc.rolling(n, min_periods=minp).mean()
    var_oc = ((log_oc - oc_mean) ** 2).rolling(n, min_periods=minp).mean()
    k = 0.34 / (1.34 + (n + 1.0) / max(1.0, n - 1.0))
    yz_var = var_overnight + k * var_oc + (1.0 - k) * rs
    return yz_var.clip(lower=0).apply(np.sqrt, raw=True)
...
_PRICE_ABS_PREFIXES = ("sma_", "ema_", "rolling_high_", "rolling_low_", "rolling_vwap_", "volume_mean_")
_CORE_PRICE_COLS = frozenset({"open", "high", "low", "close", "volume", "dollar_volume", "obv", "vpt"})


def downcast_numeric(df: pd.DataFrame) -> pd.DataFrame:
    for col in df.select_dtypes(include=["float64"]).columns:
        # Keep float64 for absolute price/volume series — float32 precision ($0.01 on $90K BTC)
        # is marginal but compounds in downstream joins; normalized features are fine as float32.
        if col in _CORE_PRICE_COLS or any(col.startswith(p) for p in _PRICE_ABS_PREFIXES):
            continue
