...
class TriggerDispatcher:
    ...
    def __init__(self, cfg: dict, state: dict, log_path: Path = TRIGGERS_LOG, spawn: bool = True):
        self.cfg = cfg; self.state = state; self.log_path = log_path; self.spawn = spawn; self._skip_log = {}; self._skip_suppressed = 0
    ...
    def dispatch(self, trig: dict) -> dict:
        ...
        if not ok:   # refus repetes (une rafale evalue un declencheur par message) : une ligne par minute et par raison
            k = f"{trig['venue']}|{trig['symbol']}|{trig['trigger_type']}|{why.split(' (')[0]}"; last = self._skip_log.get(k, 0)
            if time.time() - last < 60:
                self._skip_suppressed += 1; return rec
            self._skip_log[k] = time.time(); rec["suppressed_since_last_line"] = self._skip_suppressed; self._skip_suppressed = 0
        ...
        while not self.stop.is_set():
            try:
                ...
                for s in syms:
                    ...
                    await self.pace(1)
                    try:
                        d = await asyncio.get_event_loop().run_in_executor(None, _get, f"{FAPI}/openInterest?symbol={quote(s)}"); oi = float(d["openInterest"])
                    except Exception:
                        continue
...
def readiness(write: bool = True, publish: bool = False) -> dict:
    """publish=False (timer) : ecrit sous data_lake/market_state/readiness/ (non versionne) ; publish=True : copie versionnee sous reports/data_gap/."""
    ...
    if write:
        rp = (ROOT / "reports" / "data_gap") if publish else (TAPE_ROOT / "readiness"); rp.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    ap.add_argument("--publish", action="store_true", help="readiness : ecrire la copie versionnee sous reports/data_gap"); ap.add_argument("--dry-run", action="store_true"); ap.add_argument("--seconds", type=float); ap.add_argument("--no-capture", action="store_true", help="journaliser les declencheurs sans lancer de capture")
    a = ap.parse_args()
    if a.mode == "triggered_microstructure_capture":
        ...
        r = readiness(write=True, publish=a.publish); print(json.dumps({k: r[k] for k in ("generated_at_utc", "exchange_info", "lifecycle_events", "light_snapshots_files", "triggers")}, default=str)); print(f"windows: {len(r['windows'])} -> reports/data_gap/MARKET_STATE_TAPE_READINESS.md")
