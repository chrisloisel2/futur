...
def test_opening_long_does_not_touch_full_notional_only_fee():
    ...
    assert account.cash == cash_before - _d(10.0)
    assert account.perp_positions[PERP.key].quantity == _d(2.0)
    assert account.perp_positions[PERP.key].avg_entry_price == _d(50_000.0)
...
def test_opening_short():
    ...
    assert pos.quantity == _d(-1.5)
    assert pos.side == "SHORT"
    assert pos.avg_entry_price == _d(50_000.0)
...
def test_increasing_long_weighted_average_entry_price():
    ...
    pos = account.perp_positions[PERP.key]
    assert pos.quantity == _d(4.0)
    assert pos.avg_entry_price == _d((2 * 50_000.0 + 2 * 60_000.0) / 4.0)
...
def test_partial_reduction_realizes_pnl_and_keeps_avg_price():
    ...
    pos = account.perp_positions[PERP.key]
    assert pos.quantity == _d(3.0)
    assert pos.avg_entry_price == _d(50_000.0)       # unchanged for the remainder
    expected_realized = _d((55_000.0 - 50_000.0) * 1.0)
    assert account.cash == cash_before_close + expected_realized
...
def test_full_close_realizes_full_pnl_and_resets_avg_price():
    ...
    expected_realized = _d((48_000.0 - 50_000.0) * 2.0)             # negative
    assert account.cash == cash_before_close + expected_realized
...
def test_flip_from_long_to_short_realizes_pnl_on_old_and_opens_new_at_fill_price():
    ...
    pos = account.perp_positions[PERP.key]
    assert pos.quantity == _d(-3.0)
    assert pos.avg_entry_price == _d(52_000.0)        # fresh position, entry = fill price
    expected_realized = _d((52_000.0 - 50_000.0) * 2.0)
    assert account.cash == cash_before_flip + expected_realized
...
def test_short_pnl_sign_is_opposite_of_long():
    ...
    long_pnl = long_account.perp_positions[PERP.key].unrealized_pnl(_d(55_000.0))
    short_pnl = short_account.perp_positions[PERP.key].unrealized_pnl(_d(55_000.0))
    assert long_pnl == _d(5_000.0)
    assert short_pnl == _d(-5_000.0)
...
def test_unrealized_pnl_uses_mark_not_last_trade_price():
    ...
    assert account.perp_unrealized_pnl() == _d(3_000.0)
    assert account.nav() == account.cash + _d(3_000.0)
...
def test_funding_positive_and_negative_move_cash_directly():
    ...
    assert account.cash == cash_before + _d(12.5)
    account.apply_event(_ev(EventType.FUNDING, FundingPayload(PERP, -7.0, "USD"), "fund2"))
    assert account.cash == cash_before + _d(12.5) - _d(7.0)
