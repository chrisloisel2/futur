...
def _only_dataset(monkeypatch, name: str, spec: dict) -> None:
    ...
    isolates each test to exactly the dataset it's exercising."""
...
def test_flags_a_real_gap_beyond_tolerance(monkeypatch):
    ...
    _only_dataset(monkeypatch, "perp_5m", dict(loader=lambda s: perp_data, ts_col="timestamp", source_available_from=None))
...
def test_by_dataset_summary_counts_match_repairs(monkeypatch):
    ...
    _only_dataset(monkeypatch, "perp_5m", dict(loader=lambda s: a_data if s == "AAAUSDT" else None, ts_col="timestamp", source_available_from=None))
